...
commander_1.program.command('post').description('快捷发起post请求').action(async () => {
...
commander_1.program.command('ws').description('socket套接字测试').action(async () => {
    const { url } = await inquirer_1.default.prompt({
        type: "input",
...
commander_1.program.command('del').description('删除缓存').action(async () => {
    const keys = Object.keys(cacheJson);
    if (keys.length === 0) {
        return console.log(chalk_1.default.red('当前无缓存可以删除'));
    }
    try {
